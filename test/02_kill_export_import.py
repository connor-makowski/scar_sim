...
order = simulation.add_object(Order(
    ...
    destination_node=factory_1,
    units=1,
    planned_path=simulation.graph.get_optimal_path(supplier_0, factory_1, 'cashflow')
))
...
if simulation.orders[0].history[-1]['time'] > 2.0:
    passing = False
    err_msg = "Simulation max_time did not stop the simulation as expected."
